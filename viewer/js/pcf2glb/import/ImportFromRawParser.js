 ...
 * CAESAR II XML, CAESAR II neutral text, CAESAR II PDF input listings,
 * and STEP (.stp/.step) structural exports.
 *
...
/**
 * Convert STEP text to viewer-ready direct data.
 * Supports linear structural entities (POLYLINE and LINE).
 *
 * @param {string} stepText
 * @param {string} fileName
 * @param {object} defaults
 * @returns {{ kind:string, fileName:string, parsed:object, components:object[], messageCircleNodes:object[], messageSquareNodes:object[] }}
 */
function _stepToDirectPcfData(stepText, fileName, defaults) {
  const parsedStep = parseStpSupportMembers(stepText);
  const sectionSize = Number.isFinite(Number(defaults?.cis2SectionSizeMm))
    ? Math.max(1, Number(defaults.cis2SectionSizeMm))
    : 80;

  const components = parsedStep.members.map((member, index) => ({
    id: `stp-member-${index + 1}`,
    type: 'STEEL',
    points: [
      { x: Number(member.start.x), y: Number(member.start.y), z: Number(member.start.z), bore: sectionSize },
      { x: Number(member.end.x), y: Number(member.end.y), z: Number(member.end.z), bore: sectionSize },
    ],
    centrePoint: null,
    branch1Point: null,
    coOrds: null,
    ...
    attributes: {
      SKEY: 'STP',
      ITEM_CODE: 'STEP-STRUCT',
      'COMPONENT-IDENTIFIER': String(member.label || `MEMBER-${index + 1}`),
      SOURCE_FORMAT: 'STEP',
      SOURCE_ENTITY: String(member.sourceEntityType || ''),
      SOURCE_ENTITY_ID: String(member.sourceEntityId || ''),
    },
    source: member,
  }));
...
/**
 * Import a raw ACCDB/MDB/XML/PDF/STEP file and convert it to a viewer directPcfData object.
 *
...
export async function importFromRawFile(file, state, logArray) {
  ...
  try {
    ...
    // ── STEP text: direct structural-member import ──────────────────────────
    } else if (ext === 'stp' || ext === 'step') {
      const text = await _readText(file);
      log.push({ level: 'INFO', msg: `Read ${text.length} chars from "${fileName}"` });

      const directPcfData = _stepToDirectPcfData(text, fileName, defaults);
      log.push({
        level: 'OK',
        msg: `STEP import complete: ${directPcfData.components.length} structural member(s) built from "${fileName}".`,
      });
